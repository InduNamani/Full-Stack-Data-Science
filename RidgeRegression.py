...
st.set_page_config("Ridge Regression", layout="centered")
...
st.markdown("""
            ...
            <p>Predicting <b>California House Prices</b> based on median income, house age, average rooms, and location features with <b>L2 regularization</b>.</p>
            </div>
...
coefficients.plot(kind='barh', ax=ax, color='coral')
...
with col2:
    population = st.slider("Population", float(df['Population'].min()), float(df['Population'].max()), 1000.0)
